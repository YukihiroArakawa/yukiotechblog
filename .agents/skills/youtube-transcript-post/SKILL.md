...
4. Verify the complete, official video title from YouTube page metadata before creating the post. Do not shorten, infer, or omit title components such as episode numbers, versions, or subtitles. Normalize that exact title into a lowercase hyphenated slug by removing punctuation and replacing whitespace with hyphens, prefix it with `transcript-`, then create the template with `nix develop . -c just new-post <slug>`. For example, `Distributed Systems 1.1: Introduction` becomes `transcript-distributed-systems-1-1-introduction`.
5. Set frontmatter to `title: "[Transcript/Vocabulary] <video title>"`, include the fixed categories `english-learning` and `transcript` plus one topic-specific category, and retain the generated date and `transcript-` prefixed slug.
...
7. Use the auto-generated captions as the transcript body. Group complete caption chunks by their `start` timestamp; preserve every chunk, its order, and its wording, and join the chunks into one paragraph per timestamp section. Do not omit short connecting words or fillers. Make only minimal editorial corrections after rendering: sentence capitalization, punctuation, obvious casing, clear proper-noun corrections, and unambiguous speech-recognition errors. Do not summarize, reorder, add explanatory prose, or silently fill gaps in the captions. After every interval, add `Vocabulary` and `Phrases` tables. Vocabulary must use `Word or expression`, `IPA`, `Simple English meaning`, and `Example` columns; Phrases must use `Phrase`, `Meaning`, and `Example` columns. Give an IPA pronunciation, a simple English definition, and an original English example sentence for every vocabulary item. Each example must demonstrate the term's meaning in a concrete context; do not use generic templates such as `The lecture uses <term> when explaining the topic.` Select only terms and expressions that occur in that interval and are CEFR B1 upper-intermediate or above (approximately TOEIC 700 or above); prefer advanced general vocabulary, idioms, and technical terms over basic words and phrases.
