...
- 既存の `https://yukiotechblog.com/<slug>/` 形式のURLと同じURLで記事を表示し、既存リンクの後方互換性を保つ
...
- WordPress由来のslugは原則として変更せず、既存公開URLとの対応を維持する
...
- 記事詳細ページは既存の `https://yukiotechblog.com/<slug>/` と互換のあるURLで表示する
- 必要に応じて旧URLから新URLへのredirectまたは同一パス配信を設定する
- 代表4記事で表示確認する
